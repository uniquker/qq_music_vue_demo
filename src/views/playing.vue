<template>
	<div class="song-wrap">
		...
		<div class="song_info">
			...
			<div class="info">
				<h1 class="nowrap">{{ !!playingsong.musicData? playingsong.musicData.songname : !!playingsong.data? playingsong.data.songname : playingsong.songname}}</h1>
				<p>
	    ...
	    <div class="controller">
			<play-progress :currentTime="currentTime" :totalTime="totalTime" @showMsg="showMsgFn" v-on:switchSong='changePlayingSong'></play-progress>
	    </div>
	    
	    <alert :msg="msg" :isShow="msgIsShow" @cancel="showMsgFn" @confirm="showMsgFn"></alert>
	</div>
</template>
...
<script>
	...
	export default {
		data () {
      		...
      		let songList = store.getters.getSongList
      		//debugger;
			return {
				playingsong: song,
				songList: songList,
		...
		computed: {
			pic: function() {
				//console.log("playing page pic id is "+this.playingsong.album.mid)
				return 'http://y.gtimg.cn/music/photo_new/T002R300x300M000'+(this.playingsong.albummid?this.playingsong.albummid:this.playingsong.album.mid)+'.jpg';
			 	
			},
			songUrl: function() {
				//console.log("playing page song id is "+this.playingsong.songid)
//				return 'http://d1.stream.qqmusic.qq.com/' + this.playingsong.songid + '.m4a?fromtag=46';
				return 'http://ws.stream.qqmusic.qq.com/' + this.playingsong.songid + '.m4a?fromtag=46';
			},
			bodyStyle: function() {
				return {
					backgroundImage: "url(http://y.gtimg.cn/music/photo_new/T002R300x300M000" + (this.playingsong.albummid?this.playingsong.albummid:this.playingsong.album.mid) + ".jpg)"
				};
			},
			totalTime: function() {
		...
		methods: {
			...
			changePlayingSong: function(num) {
				...
				var playingsong = this.songList[index].data?this.songList[index].data : this.songList[index].musicData?this.songList[index].musicData: this.songList[index];
				//store.commit('changeSong', this.songlist[index].musicData);
